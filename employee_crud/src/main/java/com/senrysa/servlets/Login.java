		...
		if(! SignupMiddleware.emailValidate(email, psw)) {
			...
			try {
				...
				if(!user.getPsw().equals(null) || !user.getPsw().equals("")) {
					/*
					 * String dpsw =new String(user.getPsw()); System.out.println(dpsw);
					 * 
					 * Security.setKey(KeyGenerator.getInstance("DESede").generateKey());
					 * Security.setCipher(Cipher.getInstance("DESede"));
					 * 
					 * //Encryption //byte[] encryptionBytes = Security.encrypt(dpsw); //String
					 * passw=new String(encryptionBytes);
					 * 
					 * //System.out.println(passw); System.out.println("Œg×)~ºO".getBytes());
					 * //Decryption
